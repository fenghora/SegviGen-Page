...
function hideTexture() {
    ...
    // 把多个 model-viewer 元素放进数组
    let modelViewers = [
        document.getElementById('modelviewer1'),
        document.getElementById('modelviewer2')
    ];

    // 遍历每个 modelviewer
    modelViewers.forEach((modelViewer, idx) => {
        if (!modelViewer.model || 
            !modelViewer.model.materials[0].pbrMetallicRoughness.baseColorTexture.texture) return;

        // 如果你想分别存储每个 viewer 的状态，可以用对象数组
        if (!window_state.viewers) window_state.viewers = [];
        window_state.viewers[idx] = { textures: [], exposure: modelViewer.exposure };

        for (let i = 0; i < modelViewer.model.materials.length; i++) {
            window_state.viewers[idx].textures.push(
                modelViewer.model.materials[i].pbrMetallicRoughness.baseColorTexture.texture
            );
        }
...
function showTexture() {
    ...
    modelViewers.forEach((modelViewer, idx) => {
        ...
        // 恢复之前保存的纹理
        for (let i = 0; i < modelViewer.model.materials.length; i++) {
            modelViewer.model.materials[i].pbrMetallicRoughness.baseColorTexture.setTexture(
                window_state.viewers[idx].textures[i]
            );
        }

        // 恢复曝光度
        modelViewer.exposure = window_state.viewers[idx].exposure;
    });
}
...
function downloadGLB1() {
    let modelViewer = document.getElementById('modelviewer1');
    window.open(modelViewer.src);
}

function downloadGLB2() {
    let modelViewer = document.getElementById('modelviewer2');
    window.open(modelViewer.src);
}
...
function asset_panel_template(prompt) {
    return `
        <div class="x-section-title small-model-reviewer"><div class="x-gradient-font">Prompt</div></div>
        ...
        <div class="x-section-title small"><div class="x-gradient-font">Download</div></div>
        
        <div class="x-left-align">
            <div id="download-source" class="modelviewer-panel-button small" onclick="downloadGLB1()">Source Model</div>
            <div id="download-edited" class="modelviewer-panel-button small" onclick="downloadGLB2()">Edited Results</div>
        </div>
    `;
}

function base_asset_panel_template(prompt) {
    return `
        <div class="x-section-title small" style="margin-top: 22px;"><div class="x-gradient-font">Display Mode</div></div>
        <div class="x-left-align">
            <div id="appearance-button" class="modelviewer-panel-button small checked" onclick="showTexture()">Appearance</div>
            <div id="geometry-button" class="modelviewer-panel-button small" onclick="hideTexture()">Geometry</div>
        </div>
...
// }
// function modelviewer_window_template(item, panel, config) {
//     let viewer_size = (config && config.viewer_size) || 400;  // 每个viewer缩小一点
//     let panel_size = (config && config.panel_size) || 300;
//     let show_annotations = (config && config.show_annotations) || false;

//     let html = `<div class="x-row" style="align-items: flex-start; flex-wrap: wrap; 
//                    width: ${viewer_size * 2 + panel_size + 48}px; 
//                    max-width: calc(100vw - 32px);">`;

//     // 左边的两个model viewer并排
//     html += `<div class="x-row" style="display: flex; gap: 16px; align-items: flex-start;">`;

//     html += `
//         <div class="model-block">
//             <div class="x-section-title-model-reviewer">
//                 <div class="x-gradient-font">Source Model</div>
//             </div>
//             <div class="modelviewer-container" style="width: ${viewer_size}px;">
//                 <model-viewer
//                     id="modelviewer1"
//                     src="${item.source_model}"
//                     camera-controls
//                     tone-mapping="natural"
//                     shadow-intensity="1"
//                     environment-image="assets/env_maps/white.jpg"
//                     exposure="${item.exposure || 2}">
//                 </model-viewer>
//             </div>
//         </div>
//     `;
...
//             <div class="model-block">
//                 <div class="x-section-title-model-reviewer">
//                     <div class="x-gradient-font">Edited Results</div>
//                 </div>
//                 <div class="modelviewer-container" style="width: ${viewer_size}px;">
//                     <model-viewer
//                         id="modelviewer2"
//                         src="${item.edited_model}"
//                         camera-controls
//                         tone-mapping="natural"
//                         shadow-intensity="1"
//                         environment-image="assets/env_maps/white.jpg"
...
function modelviewer_window_template(item, panel, config) {
    ...
    html += `
            <div class="model-block">
                <div class="x-section-title-model-reviewer">
                    <div class="x-gradient-font">Segmentation Results</div>
                </div>
                <div class="modelviewer-container" style="width: ${viewer_size}px;">
                    <model-viewer
                        id="modelviewer2"
                        src="${item.segmented_model}"
                        camera-controls
                        tone-mapping="natural"
                        shadow-intensity="1"
                        environment-image="assets/env_maps/white.jpg"
...
function base_modelviewer_window_template(item, config) {
    let viewer_size = (config && config.viewer_size) || 400;  // 每个viewer缩小一点
    let panel_size = (config && config.panel_size) || 300;
    let show_annotations = (config && config.show_annotations) || false;
